# PostgreSQL 数据库导出脚本 (SQL 格式)
# 用途：将 monitoring-dashboard 数据库导出为 SQL 文件（可读格式）

param(
    [string]$OutputFile = "monitoring-dashboard-db-backup.sql"
)

Write-Host "🗄️  PostgreSQL 数据库导出工具 (SQL 格式)" -ForegroundColor Cyan
...
# 7. 执行导出 (SQL 格式)
Write-Host "`n📦 开始导出数据库 (SQL 格式)..." -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "`n❌ 导出失败" -ForegroundColor Red
        exit 1
    }
} catch {
