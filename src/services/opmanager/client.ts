...
import { OpManagerDevice, OpManagerAlarm } from './types';
...
export class OpManagerClient {
    ...
    constructor() {
        ...
        const useMockData = typeof env.USE_MOCK_DATA !== 'undefined' ? env.USE_MOCK_DATA : false;
        ...
        if (typeof window === 'undefined') {
            console.log(`🔧 OpManagerClient initialized - Mock Mode: ${this.useMock ? '✅ ENABLED' : '❌ DISABLED'}`);
            if (this.useMock) {
                console.log(`   Using Mock Data Store instead of real OpManager API`);
            } else {
                console.log(`   Connecting to OpManager at: ${typeof env.OPMANAGER_BASE_URL !== 'undefined' ? env.OPMANAGER_BASE_URL : 'http://localhost:8061'}`);
            }
        }
        ...
        const baseURL = (typeof env.OPMANAGER_BASE_URL !== 'undefined' ? env.OPMANAGER_BASE_URL : 'http://localhost:8061').replace(/\/$/, '');
        this.client = axios.create({
            baseURL,
            timeout: typeof env.OPMANAGER_TIMEOUT !== 'undefined' ? env.OPMANAGER_TIMEOUT : 60000, // 默认60秒，适应生产环境
        ...
        this.client.interceptors.response.use(
            ...
            error => {
                ...
                if (isTimeout) {
                    logger.warn(`OpManager API Timeout [${error.config?.url}]:`, errorDetails);
                } else {
                    logger.error(`OpManager API Error [${error.config?.url}]:`, errorDetails);
                }
    ...
    async getDevicesPage(options?: {
        ...
        if (this.useMock) {
            ...
            const devices = mockResponse.rows.map(row => ({
                name: row.deviceName,
        ...
        try {
            ...
            const normalizedDevices = devices.map((dev: any) => ({
                ...dev,
                ipAddress: dev.ipAddress || dev.ipaddress || 'unknown',
                name: dev.name || dev.deviceName || '',
                displayName: dev.displayName ?? dev.DisplayName ?? dev.name ?? dev.deviceName ?? '',
    ...
    async getInterfaces(options?: {
        ...
        const baseName = (options?.deviceName || options?.deviceIpAddress || '').trim();
        if (!baseName) {
        ...
        const delayMs = typeof env.OPMANAGER_GETINTERFACES_DELAY_MS !== 'undefined' ? env.OPMANAGER_GETINTERFACES_DELAY_MS : 30000;
        ...
        for (const nameParam of uniqueCandidates) {
            try {
                ...
                const now = Date.now();
                if (lastGetInterfacesTime > 0 && now - lastGetInterfacesTime < delayMs) {
                    const wait = delayMs - (now - lastGetInterfacesTime);
                    await new Promise(r => setTimeout(r, wait));
                }

                console.log(`📡 [getInterfaces] Calling API: /api/json/device/getInterfaces?name=${nameParam}`);
                ...
                if (data && typeof data === 'object' && 'error' in data) {
                    console.warn(`⚠️ [getInterfaces] OpManager returned error for name=${nameParam}:`, (data as any).error);
                    continue;
                }

                const interfaceCount = Array.isArray((data as any)?.interfaces) ? (data as any).interfaces.length : 0;
                console.log(`✅ [getInterfaces] name=${nameParam} -> Interfaces Count: ${interfaceCount}`);

                // Debug: Log if there might be more interfaces (check response structure)
                if (data && typeof data === 'object') {
                    const responseKeys = Object.keys(data);
                    if (responseKeys.includes('total') || responseKeys.includes('records') || responseKeys.includes('page')) {
                        console.warn(`⚠️ [getInterfaces] Response may have pagination metadata:`, {
                            keys: responseKeys,
                            total: (data as any).total || (data as any).records,
                ...
                if (data && (data as any).interfaces && Array.isArray((data as any).interfaces)) {
                    ...
                    return (data as any).interfaces.map((iface: any) => ({
                        ...
                        deviceName: options?.deviceName || options?.deviceIpAddress || nameParam
                    }));
                }
            } catch (error: any) {
    ...
    async getAlarms(): Promise<OpManagerAlarm[]> {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                console.warn('⚠️ [getAlarms] Unexpected response format. Expected array or object with rows/result/data array.');
                console.warn('   Response structure:', JSON.stringify(data, null, 2).substring(0, 500));
                return [];
            }

            // Log success (only in development or when explicitly enabled)
            if (process.env.NODE_ENV === 'development' || process.env.DEBUG_ALARM_SYNC === 'true') {
            ...
            return rawAlarms.map((raw: any) => ({
                ...
                severity: raw.numericSeverity?.toString() || raw.severityString || raw.severity || 'UNKNOWN', // Use numericSeverity or severityString
                name: raw.deviceName || raw.name || '', // Use deviceName from real API
                message: raw.message || '',
                modTime: raw.modTime || raw.modTimeLong ? new Date(raw.modTimeLong || raw.modTime).toISOString() : new Date().toISOString(), // Convert modTimeLong to ISO string
                category: raw.category || '',
                entity: raw.entity || ''
            }));
        } catch (error: any) {
            console.error('❌ [getAlarms] Failed to fetch alarms from OpManager:', error.message);
     ...
     */
    async getDeviceSummary(options: string | {
        deviceName?: string;
        ...
        if (this.useMock) {
            const store = getMockDataStore();
            return store.getDeviceSnapshot(deviceName || deviceIp || '');
        }
        ...
        for (const nameParam of uniqueCandidates) {
            try {
                console.log(`📊 [getDeviceSummary] Calling API: /api/json/device/getDeviceSummary?name=${nameParam}`);
                ...
                console.warn(`⚠️ [getDeviceSummary] OpManager returned error for name=${nameParam}`);
            } catch (error) {
                // 如果还有其他候选，继续尝试
     ...
     */
    async getGraphData(deviceName: string, graphName: string): Promise<any> {
     ...
     */
    async getWidgetData(dashboardName: string, widgetID: string): Promise<any> {
        if (this.useMock) {
            return {
                result: [
        ...
        try {
            const response = await this.client.get('/api/json/dashboard/getWidgetData', {
                params: {
                    dashboardName,
                    widgetID,
                    period: 1
            ...
            console.error(`Failed to fetch widget ${widgetID} from dashboard ${dashboardName}:`, error);
            return null;
        }
    }
     ...
     */
    async getBusinessView(): Promise<any> {
        if (this.useMock) {
            return {
                BusinessView: {
                    Details: [
                        { name: 'Default View_bv', displayName: 'Default View', total: '10', error: '0', alarms: '0', status: '5' }
                    ]
                }
            };
        }
     ...
     */
    async getBVDetails(bvName: string): Promise<any> {
        if (this.useMock) {
            const store = getMockDataStore();
            return store.getBusinessViewTopology(bvName);
        }

        try {
            const response = await this.client.get('/api/json/businessview/getBVDetails', {
     ...
     */
    async getBusinessDetailsView(bvName: string, startPoint: number = 0, viewLength: number = 50): Promise<any> {
        if (this.useMock) {
            const store = getMockDataStore();
